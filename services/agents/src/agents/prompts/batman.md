...
full-scale measurement of the *marginal* effect before the scale-up is committed.
...
1. **FINDINGS** — what is sound, what is wrong, what is unconfirmed.
...
Then, as the **VERY LAST line** of your reply, on its own line, with nothing after
it and no markdown formatting, emit your machine-readable verdict — exactly one of:
...
- **PASS** only if the work satisfies the FULL task spec — every requirement
  implemented AND tested AND verifiable from the proof shown, not merely "the
  existing tests pass".
- **FAIL** if anything in the spec is unmet, untested, unverifiable, or if you could
  not read a required file. A green test suite that doesn't exercise a required
  behaviour is a FAIL.

When the loop runs you automatically after a build, this line IS the gate: PASS
marks the task complete; FAIL stops it and notifies the human (it is NOT auto-sent
back to the Builder). If you are unsure, **FAIL — fail closed.** This structured
line is in addition to your prose findings above, never a replacement for them.
