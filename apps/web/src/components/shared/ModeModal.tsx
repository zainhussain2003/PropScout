/**
 * ModeModal — the routing modal shown after a user pastes a URL.
 *
 * After PropScout detects for-sale vs for-rent, this modal asks ONE question:
 *   - For-sale: "investment" or "personal"
 *   - For-rent: "tenant" or "landlord"
 *
 * The answer routes to the correct report — every section, calculation, and
 * verdict downstream is shaped by that one answer.
 *
 * Props:
 *   open      — controls visibility
 *   listing   — scraped listing preview data; null shows just the question
 *   onClose   — called on Escape, backdrop click, or the × button
 *   onSelect  — called with the chosen mode key once the loading bar finishes
 */
...
const MODE_OPTIONS: Record<'sale' | 'rent', [ModeOption, ModeOption]> = {
  sale: [
    {
      key: 'investor',
      who: 'investor',
...
function ChoiceCard({ opt, selected, dimmed, onClick }: ChoiceCardProps): JSX.Element {
...
export function ModeModal({
  ...
  const kind: 'sale' | 'rent' = listing?.kind ?? 'sale'
  ...
  // Escape key closes the modal (unless loading)
  useEffect(() => {
    if (!open) return
    ...
    >
      <div
        style={{
          ...
          // Animation: card translates up 8px and scales from 0.98→1
          animation: 'modal-card-in 0.25s ease forwards',
        }}
      >
        {/* Close button — pinned to the rounded corner */}
        <button
          onClick={onClose}
          aria-label="Close"
          disabled={loading}
        ...
        {/* Inner scroll container — its scrollbar is clipped by the rounded corners */}
        <div
          className="mm-scroll"
          style={{ overflowY: 'auto', padding: 'clamp(22px, 2.6vw, 34px)' }}
        >
          {/* Listing preview */}
          {listing !== null && <ListingPreview listing={listing} />}
          ...
          {/* Loading bar — slides in when a card is selected */}
          <div
            style={{
