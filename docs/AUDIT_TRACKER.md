...
| Google Places / Schools                            | Load-path audited 2026-07-01: `scripts/load-schools.mjs` is CSV-shape-complete and matches the `schools` table columns exactly, BUT its upsert needed a `(name, postal_code)` unique index that didn't exist — migration `20260701_add_schools_name_postal_unique.sql` written (**NOT applied**; apply before the first load). Bigger gap: **nothing reads the `schools` table yet** — the only school consumer is `googlePlacesService` (live discovery, needs `GOOGLE_PLACES_KEY`). Loading a CSV will succeed but won't reach any report until a read path (postal/catchment matching → `PersonalSchools`) is built — design that when the real data's shape is known. |
| PDF export (Puppeteer)                             | 🟡 STARTED 2026-07-01: `GET /analysis/:token/pdf` (Pro+ gated via Supabase JWT → tier; 403 `UPGRADE_REQUIRED` for free) renders the live `/r/:token` report via Puppeteer with the PropScout footer (branding + disclaimer + timestamp + token), spec §14. 6 tests. **Remaining:** frontend download wiring (StickyActionBar `onPDF` is still a no-op — needs `reportService.ts`), share-token QR code, Professional white-label branding, print-stylesheet pass so pagination matches the §14 page tables.                                                                                                                                                               |
