...
**🤖 Test 6c — Kijiji feeds and card attributes (D-120)**
`python -m pytest sources/kijiji_test.py -v` — the house and townhouse feeds follow the unfiltered feed at their own depth with each page labelled by feed; an empty page stops that feed only; a card's Bedrooms / Bathrooms / Unit type / Size items are read, the free-text bed rule is the fallback, and `raw_json.unit_type` is set only when the card says. `npx jest src/lib/compUnitType.test.ts -t D-120` — the label is read ahead of the title, and a basement or room in the title outranks it.
✋ After a nightly run: `select raw_json->>'unit_type', count(*) from rental_listings where source='kijiji' and scraped_at > now() - interval '1 day' group by 1` — House and Townhouse should each be in the dozens; the run log's per-page rows for `toronto/house` should be ~40 with none blocked.
