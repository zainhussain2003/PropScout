...
### 🔁 Regression checkpoint 1 — After PR 4 is merged

**When to run:** PR 4 wires the scraper output into the calc engine. This is the first point where the five skipped calc-engine assertions (TC-06, TC-07, TC-13, TC-14, TC-15) can be un-skipped and must pass. Run this checkpoint before PR 5 work starts. Do not start PR 5 until all items below are green.

**Before running — confirm all four are true:**

- [ ] Scraper service is running on port 3002 (`python -m uvicorn scraper_main:app --port 3002`)
- [ ] Calc engine is running on its configured port (`python -m uvicorn main:app --port 8000`)
- [ ] Calc engine `/analyse` endpoint returns a response that includes `risk_flags` array
- [ ] The five SKIP assertions in `pipeline_test.py` have been un-skipped:
  - TC-06: `calc_engine estimated_taxes == price * 0.01` (remove `skip_reason`)
  - TC-07: `pre_1980_flag == false when unknown` (remove `skip_reason`)
  - TC-07: `maintenance reserve uses standard rate when unknown` (remove `skip_reason`)
  - TC-13: `HIGH_CONDO_FEE risk flag present in analysis output` (remove `skip_reason`)
  - TC-14: `rent_controlled == true` (remove `skip_reason`)
  - TC-14: `RENT_CONTROL risk flag present` (remove `skip_reason`)
  - TC-15: `basement_unit_flag == true` (remove `skip_reason`)
  - TC-15: `VERIFY_BASEMENT_LEGALITY risk flag present` (remove `skip_reason`)

**Run the full suite:**

```
cd services/scrapers
PYTHONIOENCODING=utf-8 python pipeline_test.py
```

**Expected outcome per TC after wiring:**

| TC    | Assertion                                             | Expected                                                                      |
| ----- | ----------------------------------------------------- | ----------------------------------------------------------------------------- |
| TC-06 | `calc_engine estimated_taxes == price * 0.01`         | PASS — calc engine falls back to 1% of price when taxes unknown               |
| TC-07 | `pre_1980_flag == false when unknown`                 | PASS — no pre-1980 flag fires when year_built is unknown                      |
| TC-07 | `maintenance reserve uses standard rate when unknown` | PASS — standard 1.0% reserve used when year unknown                           |
| TC-13 | `HIGH_CONDO_FEE risk flag present in analysis output` | PASS — FIXTURE_HIGH_CONDO_FEE has fee > $600 threshold                        |
| TC-14 | `rent_controlled == true`                             | PASS — year_built_latest < 2018 on FIXTURE_PRE2018 triggers rent-control flag |
| TC-14 | `RENT_CONTROL risk flag present`                      | PASS — flag appears in risk_flags array                                       |
| TC-15 | `basement_unit_flag == true`                          | PASS — FIXTURE_BASEMENT listing description contains "basement"               |
| TC-15 | `VERIFY_BASEMENT_LEGALITY risk flag present`          | PASS — flag appears in risk_flags array                                       |

**Expected totals:**

- Total assertions: ~97
- Passing: ~94 (96%+)
- Intentional FAILs: 1 (`photo_urls is non-empty array` in TC-01 — leave as FAIL, do not skip)
- Remaining SKIPs: TC-12 (tier-gating, needs auth), TC-21 (no DI point for Supabase mock), TC-24 (school API, Week 4–5), plus all Zillow field-extraction assertions (Cloudflare bypass deferred)

**Print a delta vs the pre-PR-4 baseline (98.7% / 78 of 79 active):**

```
RESOLVED since last run:
  TC-06 calc_engine estimated_taxes            now PASS
  TC-07 pre_1980_flag == false when unknown    now PASS
  TC-07 maintenance reserve standard rate      now PASS
  TC-13 HIGH_CONDO_FEE risk flag              now PASS
  TC-14 rent_controlled == true               now PASS
  TC-14 RENT_CONTROL risk flag present        now PASS
  TC-15 basement_unit_flag == true            now PASS
  TC-15 VERIFY_BASEMENT_LEGALITY flag         now PASS

STILL FAILING (intentional):
  TC-01 photo_urls is non-empty array         FAIL (intentional — photo extraction not built)

NEW FAILURES (must be zero — any new failure is a blocker):
  (none)
```

**If any of the 8 un-skipped assertions still FAIL after wiring:**

1. Confirm the calc engine is reachable: `curl http://localhost:8000/health`
2. Print the raw calc engine response for the failing fixture URL directly:
   ```python
   import httpx, asyncio
   r = asyncio.run(httpx.AsyncClient().post("http://localhost:8000/analyse", json={"url": "<fixture_url>"}))
   print(r.json())
   ```
3. Check `risk_flags` key exists and is an array in the response — if absent, the scraper→calc wiring is incomplete
4. For TC-06/TC-07 failures: confirm `taxes_known`, `year_built_known` are being forwarded from the scraper payload to the calc engine; check the field mapping in the wiring layer
5. For TC-13/TC-14/TC-15 failures: confirm the `condo_fee`, `year_built_latest`, and `listing_description` fields are non-null in the calc engine payload before flag evaluation runs
6. Do not move to PR 5 until all 8 assertions pass

---

### 🔁 Regression checkpoint 2 — After PR 8 is merged

**When to run:** PR 8 completes the Week 3–4 frontend skeleton. This checkpoint confirms that none of the work in PR 5–8 introduced a regression in the pipeline test suite. No assertions to un-skip. The result must be identical to Checkpoint 1.

**Before running — confirm both are true:**

- [ ] Scraper service is running on port 3002
- [ ] Calc engine is running on its configured port
...
**Expected outcome:** Identical to Checkpoint 1 result — ~97 total assertions, ~94 passing (96%+), 1 intentional FAIL on `photo_urls`, same SKIPs.

**Compare against Checkpoint 1 totals. Any change is a regression:**

```
RESOLVED since Checkpoint 1:
  (none expected — no new assertions un-skipped in PR 5–8)

STILL FAILING (intentional):
  TC-01 photo_urls is non-empty array         FAIL (intentional — unchanged)

NEW FAILURES vs Checkpoint 1 (must be zero):
  (none — if anything appears here, it is a regression introduced by PR 5–8)
```

If any NEW FAILURE appears compared to Checkpoint 1, treat it as a blocker. Bisect across PR 5–8 commits to identify which change broke the assertion, fix it before launch work continues.
