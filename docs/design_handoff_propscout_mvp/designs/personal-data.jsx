// personal-data.jsx — property dataset + calc functions for the Personal Buyer report.
// One realistic Burlington semi-detached, mid-market, balanced narrative.
...
const PB_PROPERTY = {
  ...
  beds: '3', baths: '2', sqft: 1610, parking: '2 driveway',
  yearBuilt: 1972,
  lotSize: '30 × 110 ft',
  price: 875000,
  daysOnMarket: 12,
  ...
  // Carrying-cost inputs (annual unless noted)
  annualTaxes: 4280,
  condoFeeMonthly: 0,
  utilityEstMonthly: { hydro: 145, gas: 110, water: 65, internet: 65 },
  insuranceMonthlyEst: 215,

  chips: ['Personal use · For sale', 'Burlington · L7N', 'Semi-detached · 1,610 sqft', 'Built 1972 · 30 × 110 lot'],

  // FMV inputs (comparable sales)
...
// ── Schools (Burlington area, realistic mix) ─────────────────────
const PB_SCHOOLS = {
  elementary: [
    { name: 'Tom Thomson Public School',  board: 'HDSB · public', distance: '0.6 km', driveTime: '2 min', eqao: 9.1,  fraser: 88, inCatchment: true,  grades: 'JK–8' },
    { name: 'Lakeshore Public School',    board: 'HDSB · public', distance: '1.2 km', driveTime: '4 min', eqao: 8.2,  fraser: 71, inCatchment: false, grades: 'JK–8' },
    { name: 'St. Patrick Catholic School', board: 'HCDSB · catholic', distance: '1.8 km', driveTime: '5 min', eqao: 8.7,  fraser: 79, inCatchment: false, grades: 'JK–8' },
  ],
  middle: [
    { name: 'Tom Thomson PS (Gr 7–8 wing)', board: 'HDSB · public', distance: '0.6 km', driveTime: '2 min', eqao: 8.8,  fraser: 82, inCatchment: true,  grades: '7–8' },
    { name: 'Pauline Johnson PS',          board: 'HDSB · public', distance: '2.4 km', driveTime: '7 min', eqao: 7.9,  fraser: 65, inCatchment: false, grades: '7–8' },
  ],
  high: [
    { name: 'Aldershot High School',       board: 'HDSB · public',   distance: '1.4 km', driveTime: '4 min', eqao: 8.4, fraser: 76, inCatchment: true,  gradRate: 0.93, grades: '9–12' },
    { name: 'Notre Dame CSS',              board: 'HCDSB · catholic', distance: '3.2 km', driveTime: '8 min', eqao: 8.6, fraser: 81, inCatchment: false, gradRate: 0.95, grades: '9–12' },
    { name: 'M.M. Robinson HS',            board: 'HDSB · public',   distance: '4.1 km', driveTime: '10 min', eqao: 7.8, fraser: 62, inCatchment: false, gradRate: 0.88, grades: '9–12' },
  ],
};
...
// ── Neighbourhood data ────────────────────────────────────────────
const PB_NEIGHBOURHOOD = {
  walkScore: 64, transitScore: 48, bikeScore: 56,
...
// ── Home Score (personal-buyer scoring) ──────────────────────────
// 0-100 across: pricing (25), schools (20), light (15), walk/transit (15), value-add lot (15), risks (10)
function computeHomeScore(property, schools, neigh, fmv, lightScore) {
  // 1. Pricing — % vs FMV mid
  const askVsMid = (property.price - fmv.mid) / fmv.mid;  // negative = under FMV (good)
  let pricing = 0;
  if      (askVsMid <= -0.05) pricing = 25;
  else if (askVsMid <= -0.02) pricing = 22;
  else if (askVsMid <=  0.00) pricing = 18;
  else if (askVsMid <=  0.03) pricing = 14;
  else if (askVsMid <=  0.06) pricing = 8;
  else                         pricing = 0;

  // 2. Schools — average of EQAO of in-catchment schools
  const inCatch = [...schools.elementary, ...schools.middle, ...schools.high].filter((s) => s.inCatchment);
  ...
  // 5. Lot / value add
  let lotPts = 8; // baseline

  // 6. Risks (deductions taken as bonuses if absent — give 10 if no major flags)
  let riskPts = 10;
  // (could read property.riskFlags and deduct)
...
function computeMonthlyCost(property, financing) {
  ...
  // Maintenance reserve by age (in $/mo)
  const rate2 = yearBuilt >= 2010 ? 0.005 : yearBuilt >= 1980 ? 0.010 : 0.015;
  const maintenanceMonthly = price * rate2 / 12;

  const utilTotal = (utilityEstMonthly.hydro || 0)
                  + (utilityEstMonthly.gas || 0)
                  + (utilityEstMonthly.water || 0)
                  + (utilityEstMonthly.internet || 0);
  const propertyTaxMonthly = annualTaxes / 12;

  const total = monthlyMortgage + propertyTaxMonthly + condoFeeMonthly + insuranceMonthlyEst + utilTotal + maintenanceMonthly;
...
Object.assign(window, {
  PB_PROPERTY, PB_SCHOOLS, PB_COMPS, PB_NEIGHBOURHOOD,
  computeHomeScore, computeMonthlyCost,
});
