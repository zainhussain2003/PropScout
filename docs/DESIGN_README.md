...
> | `--accent-hover`    | — (did not exist)     | `#17405A` / dk `#74A5BF` | Explicit darker-harbour hover. Since 2026-09-06 `.btn-primary` rests on `--accent` and hovers to this. 10.95:1 on white.                                                                                                                                          |
> | `.btn-primary` bg   | `var(--ink)`          | `var(--accent)`          | Buttons rested on ink with harbour only on hover, so the brand colour was invisible until touched — a DOM audit found 97 of ~2,100 colour declarations carried any colour. Now 296 on the landing page. 8.94:1 light, 5.67:1 dark. (2026-09-06, DECISIONS D-025.) |
