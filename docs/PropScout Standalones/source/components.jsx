  ...
  return (
    <footer style={{ background: 'var(--ink)', color: 'var(--bg)', padding: '80px 0 36px', marginTop: 80 }}>
      <div className="container col gap-32">
        <div className="footer-grid" style={{ display: 'grid', gridTemplateColumns: '1.4fr repeat(4, 1fr)', gap: 40 }}>
          <div className="col gap-16">
            <div style={{ color: 'var(--bg)' }}>
              <Wordmark height={26} />
            </div>
            <p style={{ color: 'color-mix(in oklab, var(--bg) 60%, transparent)', fontSize: 14, maxWidth: 280 }}>
          ...
          {cols.map(c => (
            <div key={c.h} className="col gap-12">
              <div style={{ fontFamily: "'Geist Mono', monospace", fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'color-mix(in oklab, var(--bg) 50%, transparent)' }}>{c.h}</div>
              <div className="col gap-8">
                {c.items.map(i => <a key={i} href="#" style={{ color: 'color-mix(in oklab, var(--bg) 85%, transparent)', fontSize: 14 }}>{i}</a>)}
              </div>
            </div>
          ))}
        </div>

        <div className="divider" style={{ background: 'color-mix(in oklab, var(--bg) 12%, transparent)' }}></div>

        <div className="row footer-legal" style={{ justifyContent: 'space-between', fontSize: 13, color: 'color-mix(in oklab, var(--bg) 55%, transparent)', flexWrap: 'wrap', gap: 8 }}>
