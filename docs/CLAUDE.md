...
│   │
│   └── scrapers/                      # Playwright workers + FastAPI scraper service — Railway
│       ├── main.py                    # FastAPI entry — POST /scrape/listing, GET /rentals/comps
│       ├── requirements.txt           # playwright, supabase, geopy, fastapi, httpx
│       ├── setup.cfg                  # pytest / flake8 config
│       ├── __init__.py
│       │
│       ├── listing_type.py + listing_type_test.py   # URL + price-string listing type detection
│       ├── province.py + province_test.py           # FSA prefix → province, Ontario gate
│       ├── rate_limiter.py                          # File-based 4s global rate limiter
│       ├── db.py                                    # Supabase client, upsert/query helpers
│       │
│       ├── realtor_scraper.py + realtor_scraper_test.py  # httpx JSON API — TEMPLATE CODE
│       ├── zillow_scraper.py + zillow_scraper_test.py    # Playwright headless — TEMPLATE CODE
│       ├── rental_comps_scraper.py + rental_comps_scraper_test.py  # 3 sources, nightly
│       │
│       ├── routers/
│       │   ├── __init__.py
│       │   └── scraper_routes.py      # FastAPI route handlers (call scraper modules)
│       │
│       └── jobs/
│           ├── __init__.py
│           └── nightly_rental_scrape.py  # Entry point for Railway cron at 02:00 ET
│
└── supabase/
    └── migrations/                    # All schema changes — never edit DB directly in dashboard
        ├── 20260524_create_listings_tables.sql   # listings, rental_listings, scrape_logs
