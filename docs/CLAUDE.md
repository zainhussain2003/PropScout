...
│       ├── 20260531_add_analyses_table.sql
│       ├── 20260531_add_listings_table.sql
...
└── Week3-4 Front end/                 # External test suites — referenced from vite.config.ts includes
    ...
    └── PR8/                           # Mobile responsive pass tests
        ├── legal.test.tsx             # PrivacyPage, TermsPage, LegalShell scroll behaviour
        ├── bottomSheet.test.tsx       # BottomSheet open/close/escape/backdrop behaviour
        ├── stickyActionBar.test.tsx   # StickyActionBar mobile/desktop visibility + callbacks
        ├── mobileResponsive.test.tsx  # AIVerdictBlock collapse, ModeModal sheet, layout classes
        ├── routes.test.tsx            # Route wiring: /privacy, /terms, 404, /account
        └── regression.test.tsx       # Icon new icons, InvestmentMetricsSection grid, StickyActionBar on all 4 pages
```
